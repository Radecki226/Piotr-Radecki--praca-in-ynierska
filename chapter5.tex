...
\begin{itemize}
    \item $\mathrm{SNR}=20dB, \, \Delta d = 2cm$ 
    \item $\mathrm{SNR}=-10dB, \, \Delta d = 2cm$
    \item $\mathrm{SNR}=20dB, \, \Delta d = 2cm, \,$ pogłos włączony
\end{itemize}

\noindent Te same trzy wykresy zostały także wygenerowane dla $\Delta d = 20cm$.
...
\begin{itemize}
    ...
    \item Ocena działania systemu dla jednego użytkownia
...
Obliczenie SNR dla systemu jest obliczane zarówno dla systemu z pojedynczym użytkownikiem jak i z trzema użytkownikami. Aby obliczyć SNR zakłada się, że $\bm{\mathrm}g = {1,..,1}$ dla każdego indeksu czasowego. Oznacza to wyłączenie AGC. Z włączonym AGC następowałyby zmiany lokalnej amplitudy sygnału uniemożliwiając sensowny pomiar SNR. Porównanie sygnału przed odszumieniem i po odszumieniu znajduje się na \ref{fig:SNR}. Wszystkie pomiary SNR podaje się w skali logarytmicznej.
